...
class SparseSCIState:
    def __init__(self, amplitudes, ci_strs_a, ci_strs_b, norb, nelec):
        if not (len(amplitudes) == len(ci_strs_a) == len(ci_strs_b)):
            raise ValueError("Lengths of amplitudes, ci_strs_a, ci_strs_b must match")

        self.amplitudes = amplitudes      # shape (N,)
        self.ci_strs_a = ci_strs_a        # shape (N,)
        self.ci_strs_b = ci_strs_b        # shape (N,)
        self.norb = norb
        self.nelec = nelec
    ...
    *,
...
) -> tuple[float, SparseSCIState | None, tuple[np.ndarray, np.ndarray]]:
    ...
    Args:
        ...
        clean_temp_dir: Whether to delete intermediate files generated by the ``Dice`` command line application.
            These files will be stored in a directory created inside ``temp_dir``. If ``False``, then
            this directory will be preserved.
        return_sci_state: Whether to return the SCI state vector.
    ...
    print("The dice dir is: {}".format(dice_dir))
...
def solve_fermion(
    ...
    *,
    ...
    temp_dir: str | Path | None = None,
    clean_temp_dir: bool = True,
) -> tuple[float, SparseSCIState, tuple[np.ndarray, np.ndarray]]:
...
def _read_dice_outputs(
    ...
    nelec: tuple[int, int],
    return_sci_state: bool = True,
) -> tuple[float, SparseSCIState | None, np.ndarray]:
    ...
    sci_state: SparseSCIState | None = None
    if return_sci_state:

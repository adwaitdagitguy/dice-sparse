...
fn from_bin_file_to_sci<'py>(
    ...
    let amps_array = PyArray1::from_vec(py, amps);
    let dets_a_array = PyArray1::from_vec(py, dets_a);
    let dets_b_array = PyArray1::from_vec(py, dets_b);

    Ok((amps_array, dets_a_array, dets_b_array))
}

...
void SHCImakeHamiltonian::MakeHfromSMHelpers2(
  ...
  :Inputs:
      ...
      bool diskio:
          BM_description
      SparseHam& sparseHam:
          The Hamiltonian in sparse form, compose  of "Helements", "connections"
  ...
  */
  ...
  pout<<"The start index is: "<<StartIndex<<endl;
  pout<<"The end index is: "<<EndIndex<<endl;
  pout<<"DEBUG:MakeHamiltonian:diagonal elements are being added"<<endl;
  // diagonal element
  ...
  pout<<"DEBUG: MakeHamiltonian: all diagonal elements added"<<endl;

  // alpha-beta excitation
  pout<<"DEBUG: The size of AlphaMajorToBeta is: "<<AlphaMajorToBeta.size()<<endl;
  for (int i = 0; i < AlphaMajorToBeta.size(); i++) {
    long rss = getRSSkB();
    // Connections and HElems... the main reason the memory is growing... storing the full hamiltonian explicitly
    pout << "DEBUG: Iteration " << i << " RSS: " << rss/(1024*1024) << " GB "<<"RSS: "<<rss<<" kB\n";
  ...
  */
  ...
  for (int i = 0; i < AlphaMajorToBeta.size(); i++) {
    for (int ii = 0; ii < AlphaMajorToBetaLen[i]; ii++) {
      ...
      // At the end of MakeConnectionsfromSMHelpers2, before the closing brace:
    }
  }
}  // end SHCImakeHamiltonian::MakeHfromSMHelpers2
